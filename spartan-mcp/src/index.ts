...
async function main(): Promise<void> {
  ...
  const cache = createCacheManager(config.cache || {});
  ...
  const server = new Server(
    ...
    {
      capabilities: {
        tools: config.server.capabilities.tools || {},
        resources: config.server.capabilities.resources || {},
        prompts: config.server.capabilities.prompts || {}
      },
    }
  );
