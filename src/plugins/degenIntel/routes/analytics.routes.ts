...
export const analyticsRoutes: (Route | PaymentEnabledRoute)[] = [
  ...
  {
    type: 'GET',
    path: '/api/services/analytics/trending',
    public: true,
    x402: {
      priceInCents: 10,
  ...
  {
    type: 'POST',
    path: '/api/services/analytics/analyze-token',
    public: true,
    x402: {
      priceInCents: 10,
      paymentConfigs: ['base_usdc', 'solana_usdc']
  ...
  // Wallet Analytics
  {
    ...
    public: true,
    name: 'Get Wallet Analytics (Free)',
    handler: async (req: any, res: any, runtime: IAgentRuntime) => {
      try {
        const { wallet_address, chain = 'solana' } = req.query;
        ...
        const result = await (analyticsService as any).getAccountAnalytics({
        ...
        console.error('Error in wallet analytics:', error);
        res.status(500).json({
          success: false,
          error: error instanceof Error ? error.message : 'Unknown error'
        });
  ...
  {
    type: 'GET',
    path: '/api/analytics/wallet-transactions',
    public: true,
    ...
    handler: async (req: any, res: any, runtime: IAgentRuntime) => {
      try {
        ...
        if (limit && (parseInt(limit as string) < 1 || parseInt(limit as string) > 100)) {
          return res.status(400).json({
            success: false,
        ...
        if (ui_amount_mode) options.ui_amount_mode = ui_amount_mode;
        if (chain) options.chain = chain;

        const result = await (analyticsService as any).getWalletTransactionHistory(
          wallet_address,
          options
        );
        ...
        console.error('Error in wallet transaction history:', error);
        res.status(500).json({
          success: false,
          error: error instanceof Error ? error.message : 'Unknown error'
        });
  ...
  {
    type: 'POST',
    path: '/api/analytics/wallet-pnl',
    public: true,
    name: 'Get Wallet PnL Details',
    handler: async (req: any, res: any, runtime: IAgentRuntime) => {
      try {
        const {
          wallet_address,
        ...
        if (offset && (offset < 0 || offset > 10000)) {
          return res.status(400).json({
            success: false,
        ...
        // Validate token_addresses if provided
        if (token_addresses && (!Array.isArray(token_addresses) || token_addresses.length > 100)) {
          return res.status(400).json({
            success: false,
            error: 'token_addresses must be an array with maximum 100 addresses'
          });
        }
        ...
        // Build options object, only including defined values
        const options: any = {};
        if (token_addresses !== undefined) options.token_addresses = token_addresses;
        if (sort_type !== undefined) options.sort_type = sort_type;
        if (sort_by !== undefined) options.sort_by = sort_by;
        if (limit !== undefined) options.limit = limit;
        if (offset !== undefined) options.offset = offset;
        if (chain !== undefined) options.chain = chain;

        const result = await (analyticsService as any).getWalletPnLDetails(
          wallet_address,
          options
        );
        ...
        if (result) {
          ...
          res.status(500).json({
            success: false,
            error: 'Failed to fetch wallet PnL details'
          });
        }
      } catch (error) {
        console.error('Error in wallet PnL details:', error);
        res.status(500).json({
          success: false,
          error: error instanceof Error ? error.message : 'Unknown error'
        });
  ...
  {
    type: 'GET',
    path: '/api/analytics/wallet-pnl-summary',
    public: true,
    ...
    handler: async (req: any, res: any, runtime: IAgentRuntime) => {
      try {
        ...
        const result = await (analyticsService as any).getWalletPnLSummary(
          wallet_address,
          options
        );
        ...
        if (result) {
          ...
          res.status(500).json({
            success: false,
            error: 'Failed to fetch wallet PnL summary'
          });
        }
      } catch (error) {
        console.error('Error in wallet PnL summary:', error);
        res.status(500).json({
          success: false,
          error: error instanceof Error ? error.message : 'Unknown error'
        });
  ...
  // Wallet Analytics (POST, with x402 payment)
  {
    type: 'POST',
    path: '/api/services/analytics/wallet',
    public: true,
    x402: {
      priceInCents: 10,
      paymentConfigs: ['base_usdc', 'solana_usdc']
    },
    description: 'Wallet Analytics - Get comprehensive wallet analysis including portfolio, performance metrics, and trading history (swaps)',
    name: 'Analyze Wallet',
    openapi: {
      requestBody: {
        required: true,
            ...
            schema: {
              ...
              properties: {
                ...
                chain: {
                  ...
                  description: 'Blockchain network (defaults to solana)'
                }
              }
            }
          }
    ...
    handler: async (req: any, res: any, runtime: IAgentRuntime) => {
      try {
        const { wallet_address, chain = 'solana' } = req.body;
  ...
  // Wallet Analytics (GET, with x402 payment)
  {
    type: 'GET',
    path: '/api/services/analytics/wallet',
    public: true,
    ...
    validator: (req): ValidationResult => {
      return validateRequiredQueryParams(req, ['wallet_address']);
    },
    handler: async (req: any, res: any, runtime: IAgentRuntime) => {
      try {
        const { wallet_address, chain = 'solana' } = req.query;
