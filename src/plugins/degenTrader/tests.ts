...
function createMockRuntime(overrides?: Partial<IAgentRuntime>): IAgentRuntime {
  ...
  settings.set('SOLANA_PRIVATE_KEY', '4Z7crrDaQqjbqUErxiKjgAMZdaEaUKmXvdvfMQzYN1gX4Z7crrDaQqjbqUErxiKjgAMZdaEaUKmXvdvfMQzYN1gX');
  ...
  const mockSolanaService = {
    ...
    async initialize() { mockLogger.info('MockSolanaService (DegenTrader) initialized'); },
    async stop() { mockLogger.info('MockSolanaService (DegenTrader) stopped'); },
    async executeSwap(params: any) {
      mockLogger.info('MockSolanaService (DegenTrader) executeSwap called', params);
      return {
        success: true,
        signature: 'mock-signature-' + Date.now(),
        outAmount: params.outputMint === 'So11111111111111111111111111111111111111112' ? (parseFloat(params.amount) * 0.98 * 150).toString() : (parseFloat(params.amount) / 150 * 0.98).toString(), // Mock price SOL = 150 USD
        inAmount: params.amount,
        swapUsdValue: params.outputMint === 'So11111111111111111111111111111111111111112' ? (parseFloat(params.amount) * 150).toString() : (parseFloat(params.amount)).toString() // Simplified
      };
    },
    ...
    async getTokenBalance(publicKey: string, mintAddress: string) {
      mockLogger.info('MockSolanaService (DegenTrader) getTokenBalance called for', publicKey, mintAddress);
      return {
  ...
  mockLogger.info('MockSolanaService (DegenTrader) has been forcefully set on the services map for key "solana".');
  ...
  return {
    ...
    async getAgent(agentId: UUID) { return null; },
    async getAgents() { return []; },
    async createAgent(agent: any) { return true; },
    async updateAgent(agentId: UUID, agent: any) { return true; },
    async deleteAgent(agentId: UUID) { return true; },
    async ensureAgentExists(agent: any) { return agent as any; },
    async ensureEmbeddingDimension(dimension: number) {},
    async getEntityById(entityId: UUID) { return null; },
    async getEntitiesForRoom(roomId: UUID) { return []; },
    async createEntity(entity: any) { return true; },
    async updateEntity(entity: any) {},
    async getComponent(entityId: UUID, type: string) { return null; },
    async getComponents(entityId: UUID) { return []; },
    async createComponent(component: any) { return true; },
    async updateComponent(component: any) {},
    async deleteComponent(componentId: UUID) {},
    async getMemoriesByIds(ids: UUID[]) { return []; },
    ...
    async countMemories(roomId: UUID) { return 0; },
    async getCachedEmbeddings(params: any) { return []; },
    async log(params: any) {},
    async getLogs(params: any) { return []; },
    async deleteLog(logId: UUID) {},
    async createWorld(world: any) { return uuidv4() as UUID; },
    async getWorld(id: UUID) { return null; },
    async removeWorld(id: UUID) {},
    async getAllWorlds() { return []; },
    async updateWorld(world: any) {},
    async getRoom(roomId: UUID) { return null; },
    async createRoom(room: any) { return uuidv4() as UUID; },
    async deleteRoom(roomId: UUID) {},
    async deleteRoomsByWorldId(worldId: UUID) {},
    async updateRoom(room: any) {},
    async getRoomsForParticipant(entityId: UUID) { return []; },
    async getRoomsForParticipants(userIds: UUID[]) { return []; },
    async getRooms(worldId: UUID) { return []; },
    async addParticipant(entityId: UUID, roomId: UUID) { return true; },
    async removeParticipant(entityId: UUID, roomId: UUID) { return true; },
    async getParticipantsForEntity(entityId: UUID) { return []; },
    async getParticipantsForRoom(roomId: UUID) { return []; },
    async getParticipantUserState(roomId: UUID, entityId: UUID) { return null; },
    async setParticipantUserState(roomId: UUID, entityId: UUID, state: any) {},
    async createRelationship(params: any) { return true; },
    async updateRelationship(relationship: any) {},
    async getRelationship(params: any) { return null; },
    async getRelationships(params: any) { return []; },
    async getTasksByName(name: string) { return []; },
    async getMemoriesByWorldId(params: any) { return []; },
    ...
    async evaluate(message: Memory) { return null; },
    registerProvider(provider: any) { this.providers.push(provider); },
    ...
    async composeState(message: Memory) { return { values: {}, data: {}, text: '' }; },
    registerModel(modelType: any, handler: any, provider: string) {},
    getModel(modelType: any) { return undefined; },
    registerEvent(event: string, handler: any) {},
    getEvent(event: string) { return undefined; },
    async emitEvent(event: string, params: any) {},
    registerTaskWorker(taskHandler: any) {},
    getTaskWorker(name: string) { return undefined; },
    async stop() {},
...
export class DegenTraderTestSuite implements TestSuite {
  name = 'degenTrader';
  description = 'Tests for the Degen Trader plugin including services, trading logic, and integration';
  ...
  tests = [
    ...
    // Service Lifecycle Tests
    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        if (service.capabilityDescription !== 'The agent is able to trade on the Solana blockchain') {
...

    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        if (apiKeyAfterSetNull !== null) {
            throw new Error(`Test setup error: Expected API key to be null, but got: ${apiKeyAfterSetNull}`);
        }
        ...
        try {
            ...
            mockLogger.warn('WalletService initialization failed in Birdeye API key test (null case)', e);
        }
        ...
        try {
          ...
          if (!error.message.includes('Birdeye API key not found in settings or is empty')) {
            throw new Error(`Test failed (null API key): Expected Birdeye error, got: ${error.message}`);
          }
        }
        ...
        if (apiKeyAfterSetEmpty !== '') {
            throw new Error(`Test setup error: Expected API key to be empty string, but got: ${apiKeyAfterSetEmpty}`);
        }
        ...
        try {
          ...
          if (!error.message.includes('Birdeye API key not found in settings or is empty')) {
            throw new Error(`Test failed (empty API key): Expected Birdeye error, got: ${error.message}`);
          }
        }
