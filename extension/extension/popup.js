...
class SpartanDeFi {
    ...
    try {
      ...
      if (savedEmail && savedAuthToken && savedWallet) {
        try {
          ...
          this.updateDebugStatus('walletStatus', 'Connected: ' + savedWallet.slice(0, 8) + '...');
  ...
  validateWalletAddress(address) {
    ...
    if (address.length > 0 && !isValid) {
      ...
      connectBtn.disabled = address.length === 0;
    }
  }
  ...
  async selectWallet(walletAddress) {
    try {
      ...
      localStorage.setItem('spartan_wallet', walletAddress);
  ...
  async handleWalletConnect() {
    ...
    try {
      ...
      console.error('Error connecting wallet:', error);
      this.showError('Failed to connect wallet. Please check the address and try again.');
    } finally {
      this.hideLoading();
    }
  }
  ...
  renderPortfolio(portfolioData) {
    ...
    const change = totalValue * 0.05; // 5% increase
    const changePercent = 5.0;
    balanceChange.innerHTML = `
      <span class="change-amount ${change >= 0 ? 'positive' : 'negative'}">${change >= 0 ? '+' : ''}${this.formatCurrency(change)}</span>
  ...
  async handleSwap() {
    try {
      ...
      if (data.success && data.data.tokens && data.data.tokens.length > 0) {
        const firstToken = data.data.tokens[0];
